...
SDCO (Sewer Damage Classification Ontology) is an OWL ontology that formalizes the **DIN EN 13508-2** standard — the European coding system for visual inspection of drains/sewers (damage codes, characterizations, inventory/observation codes; ~80 codes total). The source standard PDF is a local, gitignored reference (not currently present in `docs/` despite `.gitignore` still listing `docs/DIN EN 13508-2 ENGLISH.pdf`) — ask the user for it if you need the primary text.
...
Full design narrative, competency questions, and SPARQL/SHACL examples: `docs/design-specs/Sewer Damage Classification Ontology SDCO.md`.
...
## Modeling approach (classes approach, merged to `main`)
...
This is the result of a **punning removal refactor**, fully executed and complete (the planning doc, formerly `docs/stale/Punning_Refactor_Plan.md`, was deleted in commit `6117b7d` once the plan was done — no longer present in the tree): originally every taxonomy term was punned (both `owl:Class` and `owl:NamedIndividual`) to fill `owl:hasValue` restrictions. That was replaced with `someValuesFrom` so subsumption reasoning works automatically without punning — trade-off: `someValuesFrom` is existential, so HermiT classifies individuals into the right damage classes but never materializes a literal, queryable triple (`:ind :hasPipeFabricDamage :CrackFissure`) for the filler.
...
- `split_mixed_disjoint_classes.py` — fixed `owl:AllDisjointClasses` blocks that wrongly mixed independent characterization dimensions (e.g. `hasCharacterization1`-keyed and `hasCharacterization2`-keyed code families listed as mutually disjoint, when a real report can legitimately be both). Same bug class as the `BAB2_A`/`hasDamageOrientation` fix that was documented in `docs/stale/Handover_Materialize_Object_Properties.md`, since deleted (commit `6117b7d`) — no longer present in the tree.
...
## Journal paper (KaSyTwin project)

SDCO is part of **KaSyTwin**, and we are now tasked with writing a journal paper on SDCO itself. `latex/main-paper/main.tex` is the paper's main file — currently a placeholder skeleton (title, author, abstract all `TODO`-style stubs) on the MDPI `journal,article` class, with sections `Introduction / Background and Related Work / Materials and Methods (Scope Definition, Metamodel Development, Taxonomy Definition, Ontology Coding, Ontology Axiomatization, Ontology Evaluation) / Results (Competency Questions, Querying and Validation) / Discussion (Benefits, Limitations) / Conclusions and Outlook`. `latex/main-paper/template.tex` is MDPI's own explanatory template — consult it for markup conventions (tables, figures, equations, citation commands) rather than guessing MDPI-class syntax.

`latex/archived-papers/` holds prior/related papers, useful as models and background, not to be edited:
- `water-17-00299-v2.pdf` — published KaSyTwin project paper (MDPI *Water*), describes the KaSyTwin project as a whole; SDCO's paper is a sibling/follow-on in the same lineage, so match its framing and terminology where relevant.
- `An-ontology-based-approach-for-sewer-infrastructure-management_Forum-Bauinformatik-2025_1021196.pdf` — related conference paper on the ontology-based sewer infrastructure approach.
- `bachelor-thesis-latex/` — the bachelor thesis that originated **m150-onto** while it was still prototypical; useful for early design rationale but predates the current punning-removal/materialization architecture described above — don't treat its modeling approach as current.

## References (journal paper)

`latex/main-paper/references.bib` — Better BibTeX auto-export ("Keep Updated") from the relevant Zotero collection; regenerates automatically whenever the collection changes, no manual re-export needed. Older `latex/bibtex-kasytwin/KaSyTwin_File.bib` is a one-off Citavi export, unrelated to this workflow.
...
- Branch discipline: `dev-classes_approach` was merged into `main` for the `0.1.0` release (2026-09-16, commit `03d92ed`); `main` now carries the full classes-approach/materialization work and is the active branch — there is no separate `dev-sdco` branch. `dev-instances_approach` remains remote-only and rejected (see above); don't revive it without raising the trade-off. Check with the user before force-pushing or rewriting history on `main`.
